Create Codex pet row `running-right` for `molly`: exactly 8 full-body frames in one horizontal strip on flat pure cyan #00FFFF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Preserve Molly exactly as the existing friendly Black Lab: floppy ears, glossy black fur, wagging tail, warm expressive face, same proportions and polished painterly 3D pet style. Upgrade only with natural look directions.. Preserve silhouette, face, palette, material, proportions, markings, and props.
